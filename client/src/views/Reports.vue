...
export default {
  ...
  setup() {
    ...
    // Reports respond to the global filter bar like every other data view.
    const {
      selectedPeriod,
      selectedLocation,
      selectedCategory,
    ...
    // Derived summary stats as computed properties (cached, recompute on data change).
    ...
    const bestQuarter = computed(() => {
      ...
      for (const q of quarterlyData.value) {
        if (!best || q.total_revenue > best.total_revenue) best = q
      }
      return best ? best.quarter : ''
    })

    // Compute the chart's max once per data change rather than on every bar render.
    ...
    // Money respects the active currency (USD/JPY conversion handled by the util).
    const formatMoney = (amount) => formatCurrency(amount, currentCurrency.value)

    // Localized month label from a "YYYY-MM" string, validated before parsing.
